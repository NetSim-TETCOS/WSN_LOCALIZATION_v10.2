...
static ANIM_HANDLE animHandle = NULL;
...
void schedule_agent_mobility(NETSIM_ID d,
							 double t,
							 double x,
							 double y,
							 double z)
{
	...
	pevent.nSubEventType = MOVE_AGENT;
	pevent.nProtocolId = MAC_PROTOCOL_IEEE802_15_4;
	ptrMOVENODE mn = calloc(1,sizeof* mn);
	mn->d = d;
	mn->time = t;
	mn->x = x;
	mn->y = y;
	mn->z = z;
	pevent.szOtherDetails = mn;
	fnpAddEvent(&pevent);
}

void add_mobility_animation()
{
	ptrMOVENODE mn = pstruEventDetails->szOtherDetails;
...
int fn_NetSim_RandomWalk(int nAgentId,double dSimulationTime,AGENT** pstruAgent)
{
	...
	for(nTimeLoop = 1;nTimeLoop<=(int)(dSimulationTime/1000000)+1;nTimeLoop++ )
	{
		...
		schedule_agent_mobility(NETWORK->nDeviceCount+(NETSIM_ID)nAgentId+1,
							pstruTempPos->dTime_sec*1000000,
							pstruTempPos->dXPos,
							pstruTempPos->dYPos,
							0);
		pstruTempPos = pstruTempPos->pstruNextPos;
...
int fn_NetSim_RandomWayPoint(int nAgentId,double dSimulationTime,AGENT** pstruAgent)
{
	...
	for(nTimeLoop = 1;nTimeLoop<=(int)(dSimulationTime/1000000)+1;nTimeLoop++)
	{
		...
		fn_NetSim_RandomPoint(pstruTempPos,&ulSeed1,&ulSeed2,pstruAgent[nAgentId]->nVelocity,pstruAgent);
		schedule_agent_mobility((NETSIM_ID)pstruAgent[nAgentId]->id,
							pstruTempPos->dTime_sec*1000000,
							pstruTempPos->dXPos,
							pstruTempPos->dYPos,
							0);
